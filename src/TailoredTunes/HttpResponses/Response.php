...
class Response
{
    ...
    private $headers = [];
    private $codeHeader = '';
    private $httpStatusCodes = [100 => "Continue", 101 => "Switching Protocols", 102 => "Processing",
                                ...
                                423 => "Locked", 424 => "Failed Dependency", 425 => "Unordered Collection",
    ...
    public function respondWith($responseCode)
    {
        $reason = $this->httpStatusCodes[$responseCode];
        $this->codeHeader = ['code' => $responseCode, 'reason' => $reason];
    }

    public function allowOriginFrom($origin = '*', $methods = ['GET', 'POST', 'OPTIONS', 'DELETE', 'PUT'])
    {
        $this->setHeaders(
            [
    ...
    {
        $this->setHeaders(['Access-Control-Allow-Credentials: true']);
    }
    ...
    public function send()
    {
        ...
        if (!empty($this->codeHeader)) {
            header($this->codeHeader['reason'], true, $this->codeHeader['code']);
        }
